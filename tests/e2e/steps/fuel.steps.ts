...
When("I select the {string} fuel type", async ({ page }, fuelType: string) => {
  ...
  const multiSelect = page.locator("button", { hasText: "All Fuel Types" })
    .first();
...
Then(
  "the metrics board should display {string} average and cheapest prices",
  async ({ page }, fuelType: string) => {
